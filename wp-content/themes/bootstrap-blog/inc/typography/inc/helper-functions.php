...
*/
function blossom_magazine_pro_is_google_font( $font ){
    $return = false;
    $websafe_fonts = blossom_magazine_pro_get_websafe_font();
    if( $font ){
...
if( ! function_exists( 'blossom_magazine_pro_get_websafe_font' ) ) {
    
    /**
     * Function listing WebSafe Fonts and its attributes
    */
    function blossom_magazine_pro_get_websafe_font(){
        ...
        return apply_filters( 'blossom_magazine_pro_standard_fonts', $standard_fonts );
    }
...
add_action( 'wp_loaded', 'blossom_magazine_pro_google_font_local' );
if( ! function_exists( 'blossom_magazine_pro_google_font_local' ) ) {
    /**
    ...
    */
    function blossom_magazine_pro_google_font_local() {

        $args = array();
        $fonts = bootstrap_blog_used_google_fonts();

        foreach( $fonts as $font ) {

            $is_google_font = blossom_magazine_pro_is_google_font( $font );
        ...
        new Blossom_Magazine_Pro_Webfonts_Local( $args );
...
if( ! function_exists( 'blossom_magazine_pro_fonts_url' ) ) {
    /**
     * Returns Google Fonts Url
    */ 
    function blossom_magazine_pro_fonts_url( $fonts = array() ) {
        ...
        foreach( $fonts as $font ) {
            ...
            if( $is_google_font ) {
                $varient = blossom_magazine_pro_check_varient( $font, 'regular', true );
...
if( ! function_exists( 'blossom_magazine_pro_check_varient' ) ) {
    /**
     * Checks for matched varients in google fonts for typography fields
    */
    function blossom_magazine_pro_check_varient( $font_family = 'serif', $font_variants = 'regular', $body = false ){
        $variant = '';
        $var     = array();
        $google_fonts  = blossom_magazine_pro_get_google_fonts(); //Google Fonts
        $websafe_fonts = blossom_magazine_pro_get_websafe_font(); //Standard Web Safe Fonts
...
if( ! function_exists( 'blossom_magazine_pro_get_google_fonts' ) ) {
    /**
     * Get Google Fonts
    */
    function blossom_magazine_pro_get_google_fonts(){
