...
if( ! class_exists( 'Bootstrap_Blog_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Bootstrap_Blog_Control_Repeater extends WP_Customize_Control {
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'bootstrap-blog' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'bootstrap-blog' ), $this->row_label['value'] );
    		}
    	...
    	public function enqueue() {    
            ...
            wp_enqueue_script( 'bootstrap-blog-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'bootstrap-blog-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'bootstrap-blog' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'bootstrap-blog' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="bootstrap-blog-image-attachment" data-placeholder="<?php echo esc_attr( 'No Image Selected', 'bootstrap-blog' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php echo esc_attr( 'No Image Selected', 'bootstrap-blog' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'bootstrap-blog' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php echo esc_attr( 'Add Image', 'bootstrap-blog' ); ?>" data-alt-label="<?php echo esc_attr( 'Change Image', 'bootstrap-blog' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php echo esc_attr( 'Change Image', 'bootstrap-blog' ); ?>
    										<# } else { #>
    											<?php echo esc_attr( 'Add Image', 'bootstrap-blog' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="bootstrap-blog-file-attachment" data-placeholder="<?php echo esc_attr( 'No File Selected', 'bootstrap-blog' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php echo esc_attr( 'No File Selected', 'bootstrap-blog' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php echo esc_attr( 'Add File', 'bootstrap-blog' ); ?>" data-alt-label="<?php echo esc_attr( 'Change File', 'bootstrap-blog' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php echo esc_attr( 'Change File', 'bootstrap-blog' ); ?>
    										<# } else { #>
    											<?php echo esc_attr( 'Add File', 'bootstrap-blog' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'bootstrap-blog' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
